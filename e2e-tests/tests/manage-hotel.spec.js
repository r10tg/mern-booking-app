...
test('should allow a user to add a hotel',async({page})=>{
    ...
    test('should display hotels',async({page})=>{
        ...
        await expect(page.getByText('Taj Skyline') ).toBeVisible()
        await expect(page.getByText("Contrary to popular belief, Lorem Ipsum")).toBeVisible();
        await expect(page.getByText('25000 per night') ).toBeVisible()
        await expect(page.getByText('5 star rating') ).toBeVisible()
        await expect(page.getByText('Luxury') ).toBeVisible()
        await expect(page.getByText('2 adults, 1 childrens') ).toBeVisible()
